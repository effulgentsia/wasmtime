...
macro_rules! ref_wrapper {
    ($wasmtime:ident => $c:ident) => {
        ...
        #[cfg(not(feature = "gc"))]
        impl From<Option<OwnedRooted<$wasmtime>>> for $c {
            fn from(_rooted: Option<OwnedRooted<$wasmtime>>) -> $c {
                $c {
                    store_id: 0,
                    a: 0,
                    b: 0,
...
pub unsafe extern "C" fn wasmtime_anyref_from_raw(
    ...
    #[cfg(not(feature = "gc"))]
    {
        let _ = cx;
        assert_eq!(raw, 0);
        crate::initialize(val, Option::<OwnedRooted<AnyRef>>::None.into());
    }
}
...
pub extern "C" fn wasmtime_anyref_from_i31(
    ...
    {
        let _ = (cx, val);
        crate::initialize(out, Option::<OwnedRooted<AnyRef>>::None.into());
    }
}
...
pub extern "C" fn wasmtime_externref_new(
    ...
    #[cfg(not(feature = "gc"))]
    {
        let _ = (cx, data, finalizer);
        crate::initialize(out, Option::<OwnedRooted<ExternRef>>::None.into());
        false
    }
}
...
pub unsafe extern "C" fn wasmtime_externref_from_raw(
    ...
    #[cfg(not(feature = "gc"))]
    {
        let _ = cx;
        assert_eq!(raw, 0);
        crate::initialize(val, Option::<OwnedRooted<ExternRef>>::None.into());
    }
}
